# 006 ? Expense Receipt Processing

## Business use case

Automate **Expense Receipt Processing** to reduce manual handoffs, improve response time, and create a traceable operational record. This blueprint is suitable for a small business or can be extended for a production team.
...
1. **Email event** ? start the workflow and capture the event with an execution timestamp.
...
5. **Execute Expense Receipt Processing** ? perform the primary operation through the relevant n8n app node or an authenticated HTTP Request node.
...
{
  "event_id": "evt_",
  "workflow": "expense-receipt-processing",
