# 005 ? Employee Onboarding Checklist

## Business use case

Automate **Employee Onboarding Checklist** to reduce manual handoffs, improve response time, and create a traceable operational record. This blueprint is suitable for a small business or can be extended for a production team.
...
1. **Form webhook** ? start the workflow and capture the event with an execution timestamp.
...
5. **Execute Employee Onboarding Checklist** ? perform the primary operation through the relevant n8n app node or an authenticated HTTP Request node.
