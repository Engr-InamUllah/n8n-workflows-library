# 003 ? Appointment Booking Confirmation

## Business use case

Automate **Appointment Booking Confirmation** to reduce manual handoffs, improve response time, and create a traceable operational record. This blueprint is suitable for a small business or can be extended for a production team.
...
5. **Execute Appointment Booking Confirmation** ? perform the primary operation through the relevant n8n app node or an authenticated HTTP Request node.
...
- A least-privilege n8n credential for Calendly, when the selected operation requires authentication
- A least-privilege n8n credential for Google Calendar, when the selected operation requires authentication
