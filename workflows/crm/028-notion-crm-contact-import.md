# 028 ? Notion CRM Contact Import

## Business use case

Automate **Notion CRM Contact Import** to reduce manual handoffs, improve response time, and create a traceable operational record. This blueprint is suitable for a small business or can be extended for a production team.
...
1. **Manual Trigger** ? start the workflow and capture the event with an execution timestamp.
...
5. **Execute Notion CRM Contact Import** ? perform the primary operation through the relevant n8n app node or an authenticated HTTP Request node.
...
- A least-privilege n8n credential for Notion, when the selected operation requires authentication
...
{
  "event_id": "evt_",
  "workflow": "notion-crm-contact-import",
