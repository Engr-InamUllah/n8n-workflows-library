# 030 ? Multi Source Lead Deduplication

## Business use case

Automate **Multi Source Lead Deduplication** to reduce manual handoffs, improve response time, and create a traceable operational record. This blueprint is suitable for a small business or can be extended for a production team.
...
5. **Execute Multi Source Lead Deduplication** ? perform the primary operation through the relevant n8n app node or an authenticated HTTP Request node.
...
- A least-privilege n8n credential for PostgreSQL, when the selected operation requires authentication
- A least-privilege n8n credential for HubSpot, when the selected operation requires authentication
- A least-privilege n8n credential for Airtable, when the selected operation requires authentication
...
{
  "event_id": "evt_",
  "workflow": "multi-source-lead-deduplication",
