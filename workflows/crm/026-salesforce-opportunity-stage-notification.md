# 026 ? Salesforce Opportunity Stage Notification

## Business use case

Automate **Salesforce Opportunity Stage Notification** to reduce manual handoffs, improve response time, and create a traceable operational record. This blueprint is suitable for a small business or can be extended for a production team.
...
1. **Salesforce record event** ? start the workflow and capture the event with an execution timestamp.
...
5. **Execute Salesforce Opportunity Stage Notification** ? perform the primary operation through the relevant n8n app node or an authenticated HTTP Request node.
...
- A least-privilege n8n credential for Salesforce, when the selected operation requires authentication
- A least-privilege n8n credential for Microsoft Teams, when the selected operation requires authentication
...
{
  "event_id": "evt_",
  "workflow": "salesforce-opportunity-stage-notification",
