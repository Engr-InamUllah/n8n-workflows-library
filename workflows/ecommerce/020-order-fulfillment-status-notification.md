# 020 ? Order Fulfillment Status Notification

## Business use case

Automate **Order Fulfillment Status Notification** to reduce manual handoffs, improve response time, and create a traceable operational record. This blueprint is suitable for a small business or can be extended for a production team.
...
1. **Fulfillment event** ? start the workflow and capture the event with an execution timestamp.
...
5. **Execute Order Fulfillment Status Notification** ? perform the primary operation through the relevant n8n app node or an authenticated HTTP Request node.
...
- A least-privilege n8n credential for Shopify, when the selected operation requires authentication
- A least-privilege n8n credential for Gmail, when the selected operation requires authentication
- A least-privilege n8n credential for Twilio, when the selected operation requires authentication
...
{
  "event_id": "evt_",
  "workflow": "order-fulfillment-status-notification",
