# 017 ? High Value Order Fraud Review

## Business use case

Automate **High Value Order Fraud Review** to reduce manual handoffs, improve response time, and create a traceable operational record. This blueprint is suitable for a small business or can be extended for a production team.
...
1. **Order event** ? start the workflow and capture the event with an execution timestamp.
...
5. **Execute High Value Order Fraud Review** ? perform the primary operation through the relevant n8n app node or an authenticated HTTP Request node.
...
{
  "event_id": "evt_",
  "workflow": "high-value-order-fraud-review",
