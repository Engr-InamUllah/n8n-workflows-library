# 012 ? WooCommerce Order to Google Sheets

## Business use case

Automate **WooCommerce Order to Google Sheets** to reduce manual handoffs, improve response time, and create a traceable operational record. This blueprint is suitable for a small business or can be extended for a production team.
...
1. **WooCommerce order event** ? start the workflow and capture the event with an execution timestamp.
...
5. **Execute WooCommerce Order to Google Sheets** ? perform the primary operation through the relevant n8n app node or an authenticated HTTP Request node.
...
{
  "event_id": "evt_",
  "workflow": "woocommerce-order-to-google-sheets",
