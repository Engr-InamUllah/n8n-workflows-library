# 011 ? Shopify Order to Slack Alert

## Business use case

Automate **Shopify Order to Slack Alert** to reduce manual handoffs, improve response time, and create a traceable operational record. This blueprint is suitable for a small business or can be extended for a production team.
...
1. **Shopify order event** ? start the workflow and capture the event with an execution timestamp.
...
5. **Execute Shopify Order to Slack Alert** ? perform the primary operation through the relevant n8n app node or an authenticated HTTP Request node.
...
{
  "event_id": "evt_",
  "workflow": "shopify-order-to-slack-alert",
