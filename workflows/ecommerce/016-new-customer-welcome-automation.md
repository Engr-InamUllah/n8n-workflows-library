# 016 ? New Customer Welcome Automation

## Business use case

Automate **New Customer Welcome Automation** to reduce manual handoffs, improve response time, and create a traceable operational record. This blueprint is suitable for a small business or can be extended for a production team.
...
1. **Customer created event** ? start the workflow and capture the event with an execution timestamp.
...
5. **Execute New Customer Welcome Automation** ? perform the primary operation through the relevant n8n app node or an authenticated HTTP Request node.
...
- A least-privilege n8n credential for WooCommerce, when the selected operation requires authentication
- A least-privilege n8n credential for Mailchimp, when the selected operation requires authentication
