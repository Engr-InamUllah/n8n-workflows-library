# 015 ? Refund Request Triage

## Business use case

Automate **Refund Request Triage** to reduce manual handoffs, improve response time, and create a traceable operational record. This blueprint is suitable for a small business or can be extended for a production team.
...
5. **Execute Refund Request Triage** ? perform the primary operation through the relevant n8n app node or an authenticated HTTP Request node.
...
- A least-privilege n8n credential for Zendesk, when the selected operation requires authentication
...
{
  "event_id": "evt_",
  "workflow": "refund-request-triage",
