# 013 ? Abandoned Cart Reminder Sequence

## Business use case

Automate **Abandoned Cart Reminder Sequence** to reduce manual handoffs, improve response time, and create a traceable operational record. This blueprint is suitable for a small business or can be extended for a production team.
...
1. **Scheduled** ? start the workflow and capture the event with an execution timestamp.
...
5. **Execute Abandoned Cart Reminder Sequence** ? perform the primary operation through the relevant n8n app node or an authenticated HTTP Request node.
...
{
  "event_id": "evt_",
  "workflow": "abandoned-cart-reminder-sequence",
